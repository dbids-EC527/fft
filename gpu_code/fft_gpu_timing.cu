/*
    nvcc -arch sm_35 fft_gpu_timing.cu -o fft_gpu_timing
    nvcc -arch compute_70 -code sm_70 fft_gpu_timing.cu -o fft_gpu_timing
...
int main(int argc, char **argv)
{
  ...
  printf("GPU kernel time, GPU overall time\n");
  long int iters = 0;
...
void runIteration(int rowLen)
{
  ...
  printf("%f, %f\n", elapsed_gpu_kernel, elapsed_gpu);
