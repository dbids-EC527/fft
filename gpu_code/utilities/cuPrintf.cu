...
__device__ static char *getNextPrintfBufPtr()
{
    ...
    if(!printfBufferPtr)
        ...
        if((restrictRules.blockid != CUPRINTF_UNRESTRICTED)
       && (restrictRules.blockid != (blockIdx.x + gridDim.x*blockIdx.y))) {
                return NULL;
        }
        if((restrictRules.threadid != CUPRINTF_UNRESTRICTED)
        && (restrictRules.threadid != (threadIdx.x + blockDim.x*threadIdx.y + blockDim.x*blockDim.y*threadIdx.z))) {
                return NULL;
        }
    ...
    // We *must* have a thread buffer length able to fit at least two
    // printfs (one header, one real)
